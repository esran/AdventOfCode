...
public class Day04 {
	private static final String Path = "day04-input.txt";
	...
	private void Load() {
		...
		data_ = reader.lines().map(Passport::MakePassport).distinct().collect(Collectors.toList());
	}

	// Count the number of complete passports
	private long Part1() {
		return data_.stream().filter(Passport::IsComplete).count();
	}

	// Count the number of valid passports
	private long Part2() {
		return data_.stream().filter(Passport::IsValid).count();
	}
